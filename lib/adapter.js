  ...
  var adapter = {
    ...
    describe: function(connectionName, collectionName, cb, noop, connection) {
      ...
      function __DESCRIBE__(connection, cb) {
        ...
        var databaseName = connectionObject.schema.hasOwnProperty('databaseName') ? mysql.escapeId(connectionObject.schema.databaseName) : false;
        var identifier = databaseName ? databaseName + '.' + tableName : tableName;

        var query = 'DESCRIBE ' + identifier;
        var pkQuery = 'SHOW INDEX FROM ' + identifier;
    ...
    define: function(connectionName, collectionName, definition, cb, noop, connection) {
      ...
      function __DEFINE__(connection, cb) {
        ...
        // Build query
        var query = 'CREATE TABLE ' + identifier + ' (' + schema + ')';
    ...
    drop: function(connectionName, collectionName, relations, cb, noop, connection) {
      ...
      function __DROP__(connection, cb) {
        ...
        function dropTable(item, next) {
          ...
          // Build query
          var query = 'DROP TABLE ' + identifier;
    ...
    addAttribute: function (connectionName, collectionName, attrName, attrDef, cb, noop, connection) {
      ...
      function __ADD_ATTRIBUTE__(connection, cb) {
        ...
        var query = sql.addColumn(identifier, attrName, attrDef);
    ...
    removeAttribute: function (connectionName, collectionName, attrName, cb, noop, connection) {
      ...
      function __REMOVE_ATTRIBUTE__(connection, cb) {
        ...
        var query = sql.removeColumn(identifier, attrName);
    ...
    create: function(connectionName, collectionName, data, cb, noop, connection) {
      ...
      function __CREATE__(connection, cb) {
        ...
        var schemaSqlOptions;
        if (databaseName) {
          schemaSqlOptions = _.extend(sqlOptions, { schemaName: databaseName });
        } else {
          schemaSqlOptions = sqlOptions;
        }
        ...
        var sequel = new Sequel(schema, schemaSqlOptions);
    ...
    createEach: function (connectionName, collectionName, valuesList, cb, noop, connection) {
      ...
      function __CREATE_EACH__(connection, cb) {
        ...
        var schemaSqlOptions;
        if (databaseName) {
          schemaSqlOptions = _.extend(sqlOptions, { shemaName: databaseName });
        } else {
          schemaSqlOptions = sqlOptions;
        }
        ...
        async.eachSeries(valuesList, function (data, cb) {
          ...
          var query = 'SELECT * FROM ' + identifier + ' WHERE ' + mysql.escapeId(pk) + ' IN (' + records + ');';
      ...
      function __JOIN__(client, done) {
        ...
        Cursor({
          ...
          $populateBuffers: function populateBuffers(options, next) {
            ...
            var schemaSqlOptions;
            if (schema.hasOwnProperty('databaseName')) {
              _schemaSqlOptions = _.extend(sqlOptions, { schemaName: _schema.databaseName });
            } else {
              _schemaSqlOptions = sqlOptions;
            }

            var sequel = new Sequel(_schema, _schemaSqlOptions);
            var _query;
    ...
    find: function(connectionName, collectionName, options, cb, noop, connection) {
      ...
      function __FIND__(connection, cb) {
        ...
        var schemaSqlOptions;
        if (schema.hasOwnProperty('databaseName')) {
          schemaSqlOptions = _.extend(sqlOptions, { schemaName: schema.databaseName });
        } else {
          schemaSqlOptions = sqlOptions;
        }
